...
class TestIntEnumFields(test.TestCase):
    ...
    async def test_values(self):
        obj0 = await testmodels.EnumFields.create(service=testmodels.Service.system_administration)
        values = await testmodels.EnumFields.all().values("service").get(id=obj0.id)
        self.assertEqual(values["service"], testmodels.Service.system_administration)

    async def test_values_list(self):
        obj0 = await testmodels.EnumFields.create(service=testmodels.Service.system_administration)
        values = await testmodels.EnumFields.all().values_list("service", flat=True).get(id=obj0.id)
...
class TestCharEnumFields(test.TestCase):
    ...
    async def test_values(self):
        ...
        values = await testmodels.EnumFields.all().values("currency").get(id=obj0.id)
    ...
    async def test_values_list(self):
        ...
        values = await testmodels.EnumFields.all().values_list("currency", flat=True).get(id=obj0.id)
