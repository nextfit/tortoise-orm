...
class TestBinaryFields(test.TestCase):
    ...
    async def test_values(self):
        ...
        values = await testmodels.BinaryFields.all().values("binary", "binary_null").get(id=obj0.id)
    ...
    async def test_values_list(self):
        ...
        values = await testmodels.BinaryFields.all().values_list("binary", flat=True).get(id=obj0.id)
