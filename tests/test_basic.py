...
class TestBasic(test.TortoiseTransactionedTestModelsTestCase):
    async def test_basic(self):
        ...
        self.assertEqual(
            await Tournament.all().order_by("id").values_list("id", flat=True),
            [tournament.id, tournament.id + 1]
        )

        self.assertEqual(
            await Tournament.all().order_by("id").values_list("pk", flat=True),
            [tournament.id, tournament.id + 1]
        )

        self.assertEqual(
            await Tournament.all().order_by("pk").values_list("pk", flat=True),
            [tournament.id, tournament.id + 1]
        )
